            ...
            let youtubeWrapper = (originalFn: Function, cooldownTimeInSeconds?: number) => {
                ...
                return function() {
                    ...
                    if (cooldownTimeInSeconds) {
                        ...
                        if (data) {
                            let momentOfAvailability: moment.Moment = moment(data, dateFormat)
                                .add(cooldownTimeInSeconds, "second");
                            let currentMoment: moment.Moment = moment();

                            if (currentMoment.isBefore(momentOfAvailability)) {
                                // tslint:disable-next-line:max-line-length
                                let errorMsg: string = `Данное действие временно недоступно. Его можно будет выполнить после ${ momentOfAvailability.format(dateFormat) }`;
